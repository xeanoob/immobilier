...
/* ─────────────────────────────────────────────────────────────
   VENDRE — Page de services vente
   
   Layout zigzag (image + texte alternés) + chiffres clés.
   ───────────────────────────────────────────────────────────── */
...
function StepBlock({ step, index }: { step: typeof STEPS[0]; index: number }) {
  ...
  return (
    <motion.div
      ...
      className={`grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-0 items-center ${index > 0 ? "mt-16 lg:mt-24" : ""}`}
    >
      {/* Image */}
  ...
  return (
    <>
      ...
      <main>
        ...
        {/* ── Chiffres clés ── */}
        <section ref={statsRef} className="bg-marine py-16 lg:py-20">
          <div className="max-w-[1400px] mx-auto px-5 lg:px-12">
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-4">
              {STATS.map((stat, i) => (
                ...
                >
                  <p className="font-display text-4xl lg:text-5xl font-800 text-bleu">{stat.value}</p>
                  <p className="font-sans text-[13px] text-blanc/60 mt-2 uppercase tracking-[0.12em]">{stat.label}</p>
                </motion.div>
              ))}
            </div>
          </div>
